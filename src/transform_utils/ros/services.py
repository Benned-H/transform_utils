...
class WaitForServiceTrigger:
    """A class that loops until its ROS service has been called."""

    def __init__(self, topic_name: str, loop_freq_hz: float = 10.0) -> None:
        """Initialize the service and then spin."""
        self.keep_looping = True
        self.service = rospy.Service(topic_name, Trigger, self.handle_trigger)
        rospy.loginfo(f"Now waiting until the service '{topic_name}' is called...")
